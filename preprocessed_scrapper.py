...
for url in all_player_url:
  ...
  for j in range (len(title)):
    if title[j].get_text() == "Full Name":
      player["Name"] = translate_text(url.rsplit("/",1)[1].rsplit("-",1)[0].replace("-"," ")) 
    elif title[j].get_text() == "Born":
      player["BirthYear"] = translate_text(content[j].get_text().rsplit(",")[1])
  ...
  with open('corpus/preprocessed_data.json', 'a', encoding='utf-8') as f:
    json.dump(player, f, ensure_ascii=False)
    f.write("\n") 

with open('corpus/data.json', 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False)

df = pd.DataFrame(data)
df.to_excel("corpus/preprocessed_data.xlsx",index=False) 
