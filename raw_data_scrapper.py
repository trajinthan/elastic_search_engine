...
for td in detail_content:
  ...
  with open('corpus/player_url.txt', 'a', encoding='utf-8') as f:
    json.dump(player_url, f)
    f.write("\n")
...
for url in all_player_url:
  ...
  for j in range (len(title)):
    if title[j].get_text() == "Full Name":
      ...
      player["DOB"] = translate_text(content[j].get_text())
    elif title[j].get_text() == "Age":
      player["Age"] = content[j].get_text()
    elif title[j].get_text() == "Batting Style":
      player["Batting Style"] = translate_text(content[j].get_text())
    elif title[j].get_text() == "Bowling Style":
      player["Bowling Style"] = translate_text(content[j].get_text())
  ...
  with open('corpus/raw_data.json', 'a', encoding='utf-8') as f:
    json.dump(player, f, ensure_ascii=False)
    f.write("\n") 

df = pd.DataFrame(data)
df.to_excel("corpus/raw_data.xlsx",index=False) 
